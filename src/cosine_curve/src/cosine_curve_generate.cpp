...
CosineCurve::CosineCurve(double pos_start, double pos_end, double vel_max, double acc_max, double v0, bool if_stop)
{
    ...
    else
    {
        ...
        V0 = v0;
        T[0] = M_PI / w;
        T[1] = (pos_dis - A * M_PI / w) / (2 * A);

        if (T[1] <= 0) // 不能达到最大速度
        {
            cout << "cannot reach max velocity!" << endl;
            vel_max = sqrt(pos_dis * acc_max / M_PI_4);
        ...
        cosineCurveGenerationWithoutDeceleclertion();
    }
}
...
void CosineCurve::cosineCurveGeneration(void)
{
    ...
    for (int i = 0; i < time_slice_sum; i++)
    {
        ...
        if (t <= T[0])
        {
            position = this->pos_start + sign * (A / w * sin(w * t + M_PI) + A * t) + V0 * t;
            velocity = sign * (A * cos(w * t + M_PI) + A) + V0;
            acceleration = sign * (-A * w * sin(w * t + M_PI));
        }
        else if (t > T[0] && t <= T[0] + T[1])
        {
            position = this->pos_start + sign * (2.0 * A * t - A * M_PI / w) + V0 * t;
            velocity = sign * 2.0 * A + V0;
            acceleration = 0;
        }
        else if (t > T[0] + T[1] && t <= T[0] + T[1] + T[2])
        {
            if (T[1] > 0) // 能到最大速度时
                position = this->pos_start + sign * (A / w * sin(w * (t - T[0] - T[1])) + A * t + pos_dis / 2 - A * M_PI / w) + V0 * t;
            else // 不能达到最大速度时
                position = this->pos_start + sign * (A / w * sin(w * (t - T[0] - T[1])) + A * t) + V0 * t;

            velocity = sign * (A * cos(w * (t - T[0] - T[1])) + A) + V0;
...
void CosineCurve::cosineCurveGenerationWithoutDeceleclertion(void)
{
    double T_sum = T[0] + T[1];
    cout << "T_sum = " << T_sum << endl;
